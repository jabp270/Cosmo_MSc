...
class ComponentSeparation:
    ...
    def observed_alms(
        ...
        # Apply instrumental response separately to CMB and foregrounds.
        cmb_T = hp.almxfl(
            alm_cmb_T,
            transfer_T)
    ...
    def component_separation(
        ...
        output_dir = self.paths.component_separation_dir(create_dir=True, mask_tag=mask_tag)
    ...
    def run_many(
        ...
        mask
            mask with apodization of simulations or None.
        ...
        for sim in range(start_sim, start_sim + nsims):
            ...
            # load observed T, E and B alms
            data = self.load_one_simulation(sim=sim)

            # alm data TEB -> map data TQU
            data_tqu = self.TEB_to_TQU(data, self.config.cmb.nside, self.config.cmb.lmax)

            # # apply mask if requested
            # if mask is not None:
            #     print(f"Applying mask to simulation {sim}")

            #     data_processed = self.apply_mask(
            #         data=data_tqu,
            #         mask=mask,
            #     )

            # else:
            #     data_processed = data_tqu
    ...
    # For include Mask and apodization

    @staticmethod
    def _alm_TEB_to_map_TQU(teb_alms, nside, lmax):
        """ 
        Convert T,E and B alms to T, Q and U maps.
        """

        return np.stack([
            hp.alm2map([teb_alms[i,0], teb_alms[i,1], teb_alms[i,2]], nside=nside, lmax=lmax, pol=True)
            for i in range(teb_alms.shape[0])
        ])
    ...
    def TEB_to_TQU(
        ...
        """ 
        Convert a SimpleNameSpace of T,E and B alms to a SimpleNameSpace of T, Q and U maps.
        """

        return SimpleNamespace(
            total = self._alm_TEB_to_map_TQU(data.total, nside, lmax),
            cmb = self._alm_TEB_to_map_TQU(data.cmb, nside, lmax),
            fgds = self._alm_TEB_to_map_TQU(data.fgds, nside, lmax),
            noise = self._alm_TEB_to_map_TQU(data.noise, nside, lmax),
            )
    
    @staticmethod
    def _apply_to_component(tqu_maps, mask):
        T_map, Q_map, U_map = tqu_maps
    ...
    def apply_mask(
        ...
        """ 
        Apply a Healpix mask/apodization window to T, Q and U maps.
        """
        return SimpleNamespace(
            total=self._apply_to_component(data.total, mask),
            cmb=self._apply_to_component(data.cmb, mask),
            fgds=self._apply_to_component(data.fgds, mask),
            noise=self._apply_to_component(data.noise, mask),
        )
